...
function pageBody(page, db, admin) {
  ...
  return `<div class="stats"><div class="stat"><small>مواعيد اليوم</small><strong>${db.appointments.filter(a => a.date === today()).length}</strong></div><div class="stat"><small>إجمالي المراجعين</small><strong>${db.patients.length}</strong></div><div class="stat"><small>مواعيد قادمة</small><strong class="accent">${db.appointments.filter(a => a.date >= today()).length}</strong></div><div class="stat"><small>الموظفون</small><strong>${db.users.filter(u => u.role === "staff").length}</strong></div></div><div class="grid-2"><div class="panel"><div class="section-title"><h2>المواعيد القادمة</h2><span class="badge">${upcoming.length} مواعيد</span></div>${upcoming.length ? upcoming.map(a => { const p = db.patients.find(x => x.id === a.patientId); const doctor = (db.doctors || []).find(x => x.id === a.doctorId); return `<div class="appointment"><div><strong>${p?.name || "مراجع"}</strong><small>${formatDate(a.date)} · باقي ${a.peopleAhead || "0"} أشخاص · ${a.type} · ${a.clinicName || "عيادة غير محددة"} · ${a.doctorName || doctor?.name || "طبيب غير محدد"}</small></div><div><span class="badge">${a.status}</span>${a.status === "مؤكد" ? `<button class="primary confirm-appointment" data-id="${a.id}" type="button">تأكيد دخول الدكتور</button>` : ""}${Number(a.peopleAhead) <= 5 && p?.phone ? `<a class="secondary whatsapp-reminder" target="_blank" rel="noopener" data-id="${a.id}" href="${whatsappLink(a, p)}">واتساب</a>` : ""}<button class="secondary edit-appointment" data-id="${a.id}" type="button">تعديل</button></div></div>`; }).join("") : '<div class="empty">لا توجد مواعيد قادمة</div>'}</div><div class="panel"><h2>تذكيرات النظام</h2><p>يتم حساب التذكير حسب الدور، ويظهر التنبيه عندما يتبقى للمراجع 5 أشخاص أو أقل.</p><div class="appointment"><div><strong>التذكير التلقائي</strong><small>مفعل لجميع المواعيد</small></div><span class="badge">مفعل</span></div></div></div>`;
}
...
function bindPageEvents(session, page) {
  ...
  if ($("#edit-appointment-form")) $("#edit-appointment-form").onsubmit = event => {
    ...
    Object.assign(appointment, { date: data.date, time: data.time, peopleAhead: Number(data.peopleAhead), doctorId: Number(data.doctorId), doctorName: data.doctorName.trim(), clinicName: data.clinicName.trim(), type: data.type, status: data.status });
  ...
  $("#appointment-form").onsubmit = event => {
    ...
    db.appointments.push({ id: Date.now() + 1, patientId: patient.id, doctorId: Number(data.doctorId), doctorName: data.doctorName.trim(), clinicName: data.clinicName.trim(), date: data.date, time: data.time, peopleAhead: Number(data.peopleAhead), type: data.type, status: "مؤكد", reminder: true });
